...
const canvasWorker = () => {
      ...
      var Enemy = function (x, y) {
        ...
        this.pushByBullet = function (bullet) {
          ...
          if (this.canBePushedByBullet) {
            ...
            if (this.health == 0) {
              // self = canvas worker environnement. 
                self.postMessage({ id: 2, data: 250 })
                this.dead = true;
            }
          }
    ...
    const tickCallback = ( currentTime ) => {
        if(!startingTime){startingTime=currentTime;}
        if(!lastTime){lastTime=currentTime;}
        totalElapsedTime=(currentTime-startingTime);
        elapsedSinceLastLoop=(currentTime-lastTime);
        lastTime=currentTime;
        // console.log('Since start: '+totalElapsedTime+ 'ms');
        // console.log('Since last loop: '+elapsedSinceLastLoop / 100 +' ms');
        
        onUpdate();
        onRender();
        self.requestAnimationFrame(tickCallback)
    }

    const setContext = ( offscreen ) => {
    ...
    self.addEventListener("message", (e) => {
        if (!e) return;
        if (e.data.id === 0) {
            setContext(e.data.data)
        }
        if (e.data.id === 1) {
            mouse = { ...mouse, x: e.data.data.x, y: e.data.data.y };
        }
        else if (e.data.id === 2) {
            mouse = { ...mouse, pressed: e.data.data };
        }
        // KeyDown
        else if (e.data.id === 3) {
        ...
        // KeyUp
          ...
          onResize(e.data.width, e.data.height);
        }
    });
}
